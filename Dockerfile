...
LABEL io.k8s.description="Platform for building and running NES rom images" \
      io.k8s.display-name="Fceux NES Emulator" \
      io.openshift.tags="builder,nes,cc65,fceux"

# Install the required software, namely fceux and build-essentials (for make)
RUN yum -y install epel-release && \
	...
	yum -y update && \
	yum install -y dejavu-lgc-sans-fonts build-essentials fceux && \
